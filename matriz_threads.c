...
void multiplicar_paralelo(float *A, float *B, float *C, int n, int num_threads) {
...
int main() {
    ...
    fprintf(arquivo, "Tamanho,Threads,TempoSequencialMedio,TempoParaleloMedio,Speedup,Eficiencia\n");
    ...
    for (int t = 0; t < qtd_tamanhos; t++) {
        ...
        for (int th = 0; th < qtd_threads; th++) {
            ...
            for (int r = 0; r < REPETICOES; r++) {
                ...
                printf("Paralelo    | n=%d | threads=%d | repeticao=%d | tempo=%.4f segundos\n",
            ...
            fprintf(arquivo, "%d,%d,%.6f,%.6f,%.6f,%.6f\n",
                    n, num_threads, media_seq, media_par, speedup, eficiencia);
